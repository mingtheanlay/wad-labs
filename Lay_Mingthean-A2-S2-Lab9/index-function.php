...
if ($conn) {
    if (isset($_GET['action'])) {
        ...
        if ($action == 0) {
            ...
            dbInsert("studentdb", $stu_name, $stu_sex, $stu_score);
            header("Location: $url");
        } else if ($action == 2) {
            $stu_id = $_GET['stu_id'];
            ...
            updateForm("studentdb", $stu_id, $stu_name, $stu_sex, $stu_score);
            $action = 3;
        } else if ($action == 3) {
            $stu_id = $_GET['stu_id'];
            $stu_name = $_GET['txt_name'];
            $stu_sex = $_GET['sel_sex'];
            $stu_score = $_GET['txt_score'];
            dbUpdate("studentdb", $stu_id, $stu_name, $stu_sex, $stu_score);
            $action = 1;
            header("Location: $url");
        }
    }
...
<body>
    ...
    <form method=get>
        ...
        if ($action == 1) {
            ...
            echo '<input type="button" value="Cancel" onclick= "window.location.href=\'index-function.php\'"> ';
        }
        ?>
    </form>
    <br />
